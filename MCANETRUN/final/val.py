...
# 加载嵌入
drug_embedding = torch.load("/Users/renhonglow/PycharmProjects/FinalYearProject/MCANETRUN/data/ligands_davis.pt").to(device)
protein_embedding = torch.load("/Users/renhonglow/PycharmProjects/FinalYearProject/MCANETRUN/data/protein_davis.pt").to(device)
...
davis_loader = DataLoader(dataset, batch_size=batch_size, shuffle=False)
...
print(f"准确率: {(correct / len(dataset)) * 100:.2f}%")
...
for idx in sample_indices:
    ...
    print(f"样本 {idx}: 真实标签={label}, 预测概率={avg_prob:.4f}, 预测标签={pred}")