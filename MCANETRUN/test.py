...
# Load embeddings
drug_embedding = torch.load("/kaggle/input/ligands-n-protein/ligands_kiba.pt").to(device)
protein_embedding = torch.load("/kaggle/input/ligands-n-protein/protein_kiba.pt").to(device)
...
print(f"drug_max_length: {drug_max_length}, drug_afterCNN: {drug_afterCNN}")
print(f"protein_max_length: {protein_max_length}, protein_afterCNN: {protein_afterCNN}")
conv = 40
...
class Model(nn.Module):
    def __init__(self, drug_embedding, protein_embedding):
        ...
        self.drug_embedding = nn.Parameter(drug_embedding, requires_grad=True).cpu()
        self.protein_embedding = nn.Parameter(protein_embedding, requires_grad=True).cpu()
    ...
    def forward(self, drug_idx, protein_idx):
        drug_idx = drug_idx.cpu() if drug_idx.device.type != 'cpu' else drug_idx
        protein_idx = protein_idx.cpu() if protein_idx.device.type != 'cpu' else protein_idx
        drug_embed = self.drug_embedding[drug_idx].to(device)
        protein_embed = self.protein_embedding[protein_idx].to(device)
...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            'drug_idx': torch.tensor(drug_idx, dtype=torch.long).cpu(),
            'protein_idx': torch.tensor(protein_idx, dtype=torch.long).cpu(),
            'label': torch.tensor(label, dtype=torch.long).to(device)
        }

dataset = Dataset('/kaggle/input/davis-n-kiba/KIBA.txt', drug_embedding, protein_embedding)
